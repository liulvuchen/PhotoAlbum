...
public class GalleryFinal {
     ...
     * 打开预览
     */
    public static void openPreview(List<PhotoInfo> data, int index) {
        FunctionConfig config = copyGlobalFuncationConfig();
        if (config != null) {
            Intent intent = new Intent(mCoreConfig.getContext(), PhotoPreviewActivity.class);
            intent.putExtra(PhotoPreviewActivity.PHOTO_LIST, (Serializable) data);
            intent.putExtra(PhotoPreviewActivity.PHOTO_INDEX, index);
            ((Activity) mCoreConfig.getContext()).startActivityForResult(intent, TAKE_REQUEST_CODE);
        } else {
            ILogger.e("Please init GalleryFinal.");
        }
    }
